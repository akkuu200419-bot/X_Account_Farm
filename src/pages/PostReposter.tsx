...
export default function PostReposter() {
  ...
  const handleRepost = () => {
    ...
    setLiveOutput(prev =>
      prev +
      `[${ts}] starting repost — platform: ${detectedPlatform}\n` +
    ...
    setTimeout(() => {
      ...
      setRunStatus('idle');
      setLiveOutput(prev =>
        prev + Array.from(selectedAccounts).map(u => `[${ts2}] @${u} → reposted\n`).join('') +
        `[${ts2}] === DONE ===\n`
      );
    }, 2000);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a] text-[#e6edf3] p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white">post_reposter</h1>
        <p className="text-[#8b949e] text-sm mt-2">Drive saved login_farm sessions to repost a post. Paste the URL, pick the accounts, hit Repost.</p>
      </div>
        ...
        <div className="flex-[0_0_35%] space-y-6">
          ...
          <div className="bg-[#0d1117] border border-[#1e2d3d] rounded-lg p-6">
            ...
            <ul className="space-y-3 text-[#8b949e] text-sm font-mono leading-relaxed">
              <li>• Sessions load from <code className="text-[#00d4ff]">~/login_farm/sessions/sessions.json</code>. Run login_farm first to create them.</li>
              <li>• Accounts list filters to the detected platform — paste a Twitter URL → only Twitter accounts shown.</li>
              <li>• <strong className="text-[#e6edf3]">Twitter/X</strong>: bare repost (no quote). Already-reposted is detected and short-circuited.</li>
              <li>• <strong className="text-[#e6edf3]">Instagram</strong>: IG has no native repost; this script does <em>Add post to your story</em> as the closest analog. Selectors are fragile.</li>
              <li>• <strong className="text-[#e6edf3]">Facebook</strong>: clicks Share → Share now (Public) | Share to News Feed.</li>
              <li>• Outcomes: <code className="text-[#00d4ff]">reposted</code> / <code className="text-[#00d4ff]">already-reposted</code> (X), <code className="text-[#00d4ff]">added-to-story</code> (IG), <code className="text-[#00d4ff]">shared</code> (FB), <code className="text-[#00d4ff]">*-not-found</code>, etc. See <code className="text-[#00d4ff]">repost.py</code>.</li>
            </ul>
          </div>
        </div>
      </div>
